...
describe('RankingCard component', () => {
  it('renders ranking card correctly', () => {
    ...
    expect(screen.getByText(PLAYERS_MOCK.players[0].name)).toBeInTheDocument();
    expect(
      screen.getByText(`${PLAYERS_MOCK.players[0].score} ${translations.en['rankingCard.points']}`),
    ).toBeInTheDocument();
    expect(
    ...
    expect(document.querySelector('.MuiLinearProgress-bar')).toHaveStyle({
      transform: `translateX(${PLAYERS_MOCK.players[0].score < GOAL ? progressPercent - 100 : 0}%)`,
    });
  });
